...
oiaApplication.factory('MasterFocusSpecificService',
	function MasterFocusSpecificService($rootScope, $state, $q, $http) {

		return {
			doSearch: function(param, callback){
				var cb = callback || angular.noop;
        		var deferred = $q.defer();
				if(angular.isUndefined(param.focusSpecific) || param.focusSpecific == ""){
					param.focusSpecific = "ALL";
				}
				var url = ENDPOINT + "master/focusSpecific/"+param.focusGroupId+"/"+param.focusSpecific;
				$http.get(url).then(
					function(success){
